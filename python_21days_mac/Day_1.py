# name = input("请输入你的名字")
# print("你好，",name)

#方法：可对数据执行的操作
#  username = "adam22"
#  print(username.upper())#全大写
#  print(username.lower())#全小写
#  print(username.title())#首字母大写
#  print(username.capitalize())#首字母大写，其余小写
#  print(username.replace("a", "A"))#替换
#  print(username.find("a"))#查找字符位置
#  print(username.index("a"))#查找字符位置
#  print(username.count("a"))#统计字符出现次数
#  print(username.startswith("a"))#判断是否以某个字符开头
#  print(username.endswith("m"))#判断是否以某个字符结尾
#  print(username.isalpha())#判断是否全是字母 

#在字符串中插入变量:用f“{变量1}{变量2}”
# first_name = "zhou"
# last_name = "jinsheng"
# full_name = f"{first_name} {last_name}"
# print(full_name)
# #在字符串中插入变量:用format方法
# full_name2 = "{} {}".format(first_name, last_name)
# print(full_name2)
# #在字符串中插入变量:用%格式化
# full_name3 = "%s %s" % (first_name, last_name)
# print(full_name3)  
# #在字符串中插入变量:用join方法
# full_name4 = " ".join([first_name, last_name])
# print(full_name4)
  
#使用字表符添加空白 
# favorite_language = "    python.  1  "
# print(favorite_language)
# favorite_language = favorite_language.lstrip()#去除空格
# print(favorite_language)

# Name = input("请输入你的名字：")
# print("hello",Name,"请继续加油啊！")

2+3
